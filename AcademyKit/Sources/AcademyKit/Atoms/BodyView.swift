...
public struct BodyView: View {
    ...
    public var body: some View {
        Text(text)
            .font(.custom(weight == .regularWeight && italic == false ? "Cygre-Regular" : weight == .regularWeight && italic == true ? "Cygre-RegularIt" : weight == .boldWeight && italic == false ? "Cygre-Bold" : "Cygre-BoldIt", size: 17))
    }
}


#Preview {
    BodyView(text: "Body",weight:.regularWeight, italic: true)
}
